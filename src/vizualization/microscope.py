...
from src.print_utils import LABEL, SHORT_LABEL
...
def plot_microscope_for_different_models(base, num_candidates, min_pop=None, max_pop=None, with_ic=False,
    ...
    for sampler_name in base:
        ...
        # if title is None:
        title = f'{SHORT_LABEL.get(sampler_name, sampler_name)}'
...
def microscope_colored_by_position_of_candidate(
        election,
        title,
        saveas,
        candidate_id=0,
    ...
    # have gradient colors of a standard colormap
    ...
    for i in range(len(election.coordinates['vote'])):
        ...
        pote = vote_to_pote(election.votes[i])
        election.microscope.ax.scatter(election.coordinates['vote'][i][0],
                                       election.coordinates['vote'][i][1],
                                       alpha=0.67,
                                       s=80,
                                       color=COLORS[pote[candidate_id] % len(COLORS)]
